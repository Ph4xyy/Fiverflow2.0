-- Script corrigé pour vérifier les commandes avec les bonnes colonnes
-- Ce script vérifie que les commandes ont des due_date et budget et peuvent apparaître dans le calendrier
...
-- 7. Vérifier les colonnes de budget dans orders
SELECT 
  'Colonnes de budget dans orders' as info,
  column_name,
  data_type
FROM information_schema.columns 
WHERE table_name = 'orders'
  AND (column_name LIKE '%budget%' OR column_name LIKE '%amount%' OR column_name LIKE '%price%')
