...
import { welcomePageContent } from '../../lib/docsPageContent';
  ...
  return (
    ...
    >
      <h1 className="text-3xl md:text-4xl font-semibold tracking-[-0.04em] text-white mb-4 !mt-0">
        <span className="bg-gradient-to-r from-indigo-400 via-purple-400 to-cyan-300 bg-clip-text text-transparent">Welcome</span>
      </h1>
      <p className="text-neutral-300 leading-relaxed text-base md:text-lg mb-8 !text-current">
        FiverFlow is your all-in-one dashboard designed to help freelancers organize their clients, manage tasks, and track growth with ease.
      </p>
      {welcomePageContent.map((section, index) => (
        <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
          <h2 className="text-xl md:text-2xl font-semibold text-white mt-10 mb-3 !text-current">{section.heading}</h2>
          <div className="space-y-4 !text-neutral-300">
            {section.body.map((paragraph, pIndex) => (
              <p key={pIndex} className="leading-relaxed !text-current">{paragraph}</p>
            ))}
          </div>
          {section.callout && (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12 pt-8 border-t border-white/10">
        <div className="flex flex-wrap gap-4">
          <button className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-white/20 bg-white/5 text-white hover:bg-white/10 transition-colors">
            <ArrowLeft size={18} /> Previous Tutorial
          </button>
