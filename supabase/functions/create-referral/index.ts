...
Deno.serve(async (req) => {
  try {
    ...
    const { referrer_code, user_id }: ReferralRequest = await req.json();

    // Validate input
    if (!referrer_code || !user_id) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Missing referrer_code or user_id'
        }),
        { 
          status: 400,
    ...
    console.log(`🔗 Processing referral for user ${user_id} with code ${referrer_code}`);
