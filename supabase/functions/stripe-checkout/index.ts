...
Deno.serve(async (req) => {
  try {
    ...
    if (!customer || !customer.customer_id) {
      ...
      // Note: We don't check or create subscription records here because the subscription doesn't exist yet
      // The subscription will be created by Stripe webhook after successful payment
    }
    ...
    if (mode === 'subscription' && trial_period_days && trial_period_days > 0) {
      console.log(`Adding ${trial_period_days} day trial to subscription`);
    ...
    console.log(`Created checkout session ${session.id} for customer ${customerId}${trial_period_days ? ` with ${trial_period_days} day trial` : ''}`);
